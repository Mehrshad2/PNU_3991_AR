...
\begin{center}
	...
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw (0) to node [left=-1pt] {e, 1} (5.center);
		...
		\draw [style=new edge style 0] (1) to node [above=-3pt] {e} (2);
		\draw [style=new edge style 0] (3) to node [above=-13pt] {1} (4);
...
\end{tikzpicture}
\end{center}
 \textit{\textbf{Solution:}} This problem can be solved by the $\in$ -closure method.\\$\in$-closure(1) = \{1, 4\} \hspace*{1cm} $\in$-closure(3) = \{3\} \hspace*{1cm} $\in$-closure(5) = \{5\} \\
 $\in$-closure(2) = \{2, 3\} \hspace*{1cm} $\in$-closure(4) = \{4\} \\As 1 is the beginning state,start with $\in$-closure(1) = {1,4}.Let us rename it as A.
 A is still unmarked.\\Then, construct a δ' function for the new unmarked state A for inputs 0 and 1.\\
 δ'(A, 0) = $\in$-closure(δ (A, 0))\hspace*{2cm}δ'(A, 1) = $\in$-closure(δ (A, 1))\\
\hspace*{1.2cm} = $\in$-closure (δ ((1,4), 0))\hspace*{2.6cm} = $\in$-closure (δ ((1, 4), 1))\\
...
It is a new state. Mark it as E.\\
 δ'(E, 0) = $\in$-closure(δ (E, 0)) \hspace*{1.8cm}δ'(E, 1) = $\in$-closure(δ (E, 1))\\
 \hspace*{1.2cm} = $\in$-closure (δ (3), 0) \hspace*{3cm}= $\in$-closure (δ (5), 1)\\
 \hspace*{1.2cm} = $\emptyset$ \hspace*{5.7cm}= $\in$-closure (5)\\
 \hspace*{7.7cm}= {5} = D\\
...
1. The machine format of regular expression is:\hspace*{0.1cm}a) Finite automata \hspace*{0.1cm}b) Push down automata \hspace*{0.2cm}c) Turing machine\hspace*{0.5cm}d) All of the above\\
 2. The regular expression is accepted by:\hspace*{0.5cm}a) Finite automata\hspace*{0.5cm}b) Push down automata\hspace*{0.5cm}c) Turing machine\hspace*{0.5cm}d) All of the above\\
3. The language of all words with at least 2 a’s can be described by the regular expression:\hspace*{0.2cm}a) (ab)*a\hspace*{0.2cm}b) (a + b)*ab*a(a + b)*\hspace*{0.2cm} c) b*ab*a(a + b)*\hspace*{0.2cm}d) All of the above\\
 4. The set of all strings of {0, 1} having exactly two 0’s is:\hspace*{0.2cm}
 a) 1*01*01*\hspace*{0.2cm} b) {0 + 1)*1\\c) {11 + 0}*\hspace*{0.5cm}d) {00 + 11}*
 \end{document}
